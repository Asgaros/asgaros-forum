    ...
    function render_feed() {
        if ($this->asgarosforum->options['enable_rss'] && !empty($_GET['showfeed'])) {
            ...
            if ($this->asgarosforum->current_view === 'forum') {
                ...
                $feed_data = $this->asgarosforum->db->get_results("SELECT t.id, t.name, ({$query_post_content}) AS text, ({$query_post_date}) AS date, ({$query_post_author}) AS author_id FROM {$this->asgarosforum->tables->topics} AS t WHERE t.parent_id = {$this->asgarosforum->current_forum} ORDER BY t.id DESC LIMIT 0, 50;");
            } else if ($this->asgarosforum->current_view === 'topic') {
