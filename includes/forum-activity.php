    ...
    public function show_activity() {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $activity) {
                ...
                // Check if the user is logged in before displaying the avatar
                if (is_user_logged_in()) {
                    $avatar = get_avatar($activity->author_id, 32); // Change the size (32) as needed
                } else {
                    $avatar = ''; // Don't display avatar for logged-out users
                }
                ...
                if ($this->asgarosforum->is_first_post($activity->id, $activity->parent_id)) {
                    ...
                    echo $avatar; // Display avatar only if user is logged in
