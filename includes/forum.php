...
class AsgarosForum {
    ...
    public function escape_script_tags($string) {
        // Remove any invalid control characters from the given string.
        $string = wp_kses_no_null($string, array('slash_zero' => 'keep'));

        // Convert and fix HTML entities.
        $string = wp_kses_normalize_entities($string);

        return preg_replace_callback('%(<!--.*?(-->|$))|(<[^>]*(>|$)|>)%', array($this, 'escape_script_tags_callback'), $string);
    }

    function escape_script_tags_callback($match) {
        $string = $match[0];
        $string = wp_kses_stripslashes($string);

        // Cancel if it matched a ">" character.
        if (substr($string, 0, 1) != '<') {
            return $string;
        }

        // Cancel if it matched a comment.
        if ('<!--' == substr($string, 0, 4)) {
            return $string;
        }

        // Cancel if it matched a seriously malformed construct.
        ...
        // Ensure that iframe-tag is allowed.
        if (!isset($allowed_html['iframe'])) {
            $allowed_html['iframe'] = array();
        }

        // Escape tag, if it is a not allowed HTML element.
