	...
	public static function getFileList($post_id, $uploads, $frontend = false) {
        ...
        if (!empty($uploads) && is_dir($path)) {
			...
			if ($frontend && !is_user_logged_in() && $asgarosforum->options['hide_uploads_from_guests']) {
				$upload_list_elements .= '<li>'.__('You need to login to have access to uploads.', 'asgaros-forum').'&nbsp;<a href="'.esc_url(wp_login_url($asgarosforum->getLink('current'))).'">&raquo; '.__('Login', 'asgaros-forum').'</a></li>';
			} else {
